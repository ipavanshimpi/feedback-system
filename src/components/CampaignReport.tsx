...
export function CampaignReport({ id, onBack }: CampaignReportProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-100 pb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 py-2 px-4 rounded-xl border border-red-200 hover:bg-red-50 text-xs font-semibold text-red-650 hover:text-red-750 transition active:scale-[0.98] cursor-pointer font-mono text-[11px]"
          >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-[999] animate-fade-in">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-neutral-150/80 animate-scale-up">
            <div className="text-center space-y-3">
              <div className="w-12 h-12 bg-red-50 text-red-650 rounded-2xl flex items-center justify-center mx-auto border border-red-100 shadow-sm shadow-red-500/5">
                <Trash2 className="w-6 h-6 stroke-[1.5]" />
              </div>
              <h3 className="text-xl font-black text-neutral-950 tracking-tight">Delete Feedback Program?</h3>
              <p className="text-[10px] text-red-500 font-mono tracking-widest uppercase font-bold">WARNING: IRREVERSIBLE ACTION</p>
            </div>

            <p className="text-neutral-500 text-sm text-center leading-relaxed">
              Are you sure you want to delete <strong className="text-neutral-900 font-bold">"{data?.campaign.title}"</strong>? 
              This will permanently delete this program and all associated feedback responses from students. This action cannot be undone.
            </p>

            {deleteError && (
              <p className="text-xs text-red-500 font-semibold font-mono text-center bg-red-50 p-2.5 rounded-xl border border-red-100">
                {deleteError}
              </p>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 pt-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-3 px-4 rounded-xl border border-neutral-200 hover:bg-neutral-50 text-neutral-700 font-bold text-xs uppercase tracking-wider font-mono transition active:scale-[0.98] cursor-pointer text-center"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 py-3 px-4 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs uppercase tracking-wider font-mono transition active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
              >
                {deleting ? (
                  <>
